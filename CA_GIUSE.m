...
%     Letta sul grafico di nichols, pulsazione per cui |Lz|=0db
    ...
    Rd=1; Ri=1; Rpi=1;% NO EDIT
    FAA_ON = 0;        % ABILITA FILTRO SE POSTO A 1
    ...
    % Reti derivative, fase guadagnata in funzione dell'Md. Automaticamente
    % viene determinata la normalizzazione migliore.
    md=4; md2=8;
    % Valore md, e fase recuperata:
    ...
    % Reti integrative, Automaticamente viene determinata la normalizzazione migliore.
    mi = 9; mi2= 1; 
...
%% [ 5.1 ] Rete correttive 
    ...
    if  md ~=1
        zd=wprog/(chop(sqrt(md),2,1)); % norm_rd=1.2; zd = wprog/norm_rd;
    ...
    if  md2 ~=1
        zd2=wprog/(chop(sqrt(md2),2,1)); % norm_rd2=1.2; zd2 = wprog/norm_rd2; 
    ...
    if mi ~=1
        pii=1/(max((100/wprog),((10*mi)/(wprog)))); %%norm_ri=98; pii = wprog/norm_ri;
    ...
    if mi2 ~=1
        pii2=1/(max((100/wprog),((10*mi2)/(wprog)))); %%norm_ri2=0; pii2 = wprog/norm_ri2;
