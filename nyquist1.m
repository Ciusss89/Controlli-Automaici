...
ni=nargin;
if ni==0, eval('exresp(''nyquist'')'), return, end
...
elseif (ni==2),     % Transfer function form without frequency vector
        num  = a; den = b; 
        w = freqint2(num,den,30);
        [ny,nn] = size(num); nu = 1;

elseif (ni==3),     % Transfer function form with frequency vector
        num = a; den = b;
        w = c;
        [ny,nn] = size(num); nu = 1;
...
elseif (ni==4),     % State space system, w/o iu or frequency vector
        ...
        [iu,ni,re,im]=mulresp('nyquist',a,b,c,d,w,nargout,0);
